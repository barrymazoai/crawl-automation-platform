  ...
  const brandLinkReconcilers = config.brandLink
    ? (() => {
        ...
        return config.brandLink!.channels.map((channel) => new BrandLinkReconciler(
          ...
          { channel, catalogMaxAgeMs: config.brandLink!.catalogMaxAgeMs, enqueuePerTick: config.brandLink!.enqueuePerTick, enqueueAmbiguous: config.brandLink!.enqueueAmbiguous },
