...
export async function runAmazonPipeline(options: AmazonPipelineOptions): Promise<AmazonPipelineResult> {
  ...
  await writeJson(path.join(options.jobDirectory, "amazon", "discovery.json"), discovery);
  if (discovery.asins.length === 0) {
    return { status: "needs_review", reasonCode: "amazon_no_asin", summary: "Amazon 页面没有发现 ASIN", itemCount: 0 };
  }

  const capture = await captureProducts(options, discovery.asins);
  ...
  const unify = await unifyProducts(options, included, semanticByAsin);
  const unifyByAsin = new Map(unify.results.map((result) => [result.clientRef, result]));
  const blocking: string[] = [];
  const domainCache = new Map<string, string | null>();
  const normalized: NormalizedProduct[] = [];
  const facts: ProductFacts[] = [];

  const productConcurrency = Math.min(2, Math.max(1, options.ocrConcurrency));
  const perProductOcrConcurrency = Math.max(1, Math.floor(options.ocrConcurrency / productConcurrency));
  const factsResults = await mapWithConcurrency(included, productConcurrency, async (product) => [
    product.asin,
    await extractFacts(
      { ...options, ocrConcurrency: perProductOcrConcurrency },
      product,
      (semanticByAsin.get(product.asin)?.ingredients.length ?? 0) === 0,
    ),
  ] as const);
  const factsByAsin = new Map(factsResults);

  for (const product of included) {
    assertNotAborted(options.signal);
    const semanticResult = semanticByAsin.get(product.asin)!;
    const unified = unifyByAsin.get(product.asin);
    if (!unified) { blocking.push(`${product.asin}: Product Unify 没有形成合法名称与变体结果`); continue; }
    const extractedFacts = factsByAsin.get(product.asin)!;
    if (extractedFacts.review) { blocking.push(extractedFacts.review); continue; }
    const factsRecord = extractedFacts.facts;
    const hasFormulaEvidence = semanticResult.ingredients.length > 0
      || extractedFacts.imageIngredients.length > 0
      || Boolean(factsRecord?.rows.some((row) => row.isActive !== false && row.name.trim()));
    if (!hasFormulaEvidence) {
      blocking.push(`${product.asin}: 页面文字和标签图片都没有形成可验证的配方成分`);
      continue;
    }
    ...
    normalized.push(normalizeAmazonProduct(product, semanticResult, unified, factsRecord, extractedFacts.imageIngredients, domain, options.runId, new URL(options.url).origin, scopeDecision.scope));
    if (factsRecord) facts.push(factsRecord);
  }
  ...
  if (blocking.length > 0) {
    await writeJson(path.join(options.jobDirectory, "amazon", "review.json"), { blocking, warnings: [...semantic.warnings, ...unify.problems] });
    return {
      status: "needs_review",
  ...
  return {
    ...
    summary: `Amazon 完成：发现 ${products.length}，入库并回读 ${normalized.length}，Facts ${facts.length}，scope=${scopeDecision.scope}${scopeDecision.reasons.length ? `（${scopeDecision.reasons.join(", ")}）` : ""}`,
  };
}
