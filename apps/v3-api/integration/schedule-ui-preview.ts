...
import { scheduleActivities } from "../src/schedules/activities.js";
import { scheduleIdFor, type ScheduleTick } from "@crawl-automation/v3-contracts";
...
const scheduleWorkers: Worker[] = [];
const scheduleRuns: Promise<void>[] = [];
...
try {
  env = await TestWorkflowEnvironment.createLocal({ server: { ip: "127.0.0.1", ui: true, uiPort: 8235,
  ...
  const brand = (await brands.create({ name: "计划验收 · 合成 Brand", note: "CRAWLV3-13 隔离验收；不是实际采集" }, randomUUID())).value;
  ...
  const gateway = new TemporalGateway(env.client, { clusterId: "crawlv3-13-local", namespace: env.client.workflow.options.namespace, taskQueue: `ui-probe-${randomUUID()}`, workflowType: "DeliveryAcceptanceProbe" });
  let paused = false, unavailable = false, rejectNext = false;
  ...
  const scheduleQueue = `schedule-intake-${randomUUID()}`, activityQueue = `schedule-accept-${randomUUID()}`;
  const scope = { clusterId: gateway.target.clusterId, namespace: gateway.target.namespace, activityQueue };
  const schedules = new TemporalSchedules(env.client, { ...scope, taskQueue: scheduleQueue }, scheduleSourceReader(db.pool));
  const activity = scheduleActivities(submissions, scope);
  scheduleWorkers.push(await Worker.create({ connection: env.nativeConnection, namespace: scope.namespace, taskQueue: scheduleQueue,
    workflowsPath: fileURLToPath(new URL("../src/schedules/workflow.ts", import.meta.url)), identity: `schedule-workflow@${process.pid}` }));
  scheduleWorkers.push(await Worker.create({ connection: env.nativeConnection, namespace: scope.namespace, taskQueue: activityQueue,
    identity: `schedule-accept@${process.pid}`, activities: { acceptScheduleTick: async (tick: ScheduleTick) => {
      const result = await activity.acceptScheduleTick(tick); event({ event: "SCHEDULE_TICK", tick, result }); return result;
    } } }));
  for (const w of scheduleWorkers) { const running = w.run(); running.catch(error => { event({ event: "SCHEDULE_WORKER_ERROR", message: String(error) }); controller.abort(); }); scheduleRuns.push(running); }
  const token = randomBytes(32).toString("hex");
  const api = createApp(brands, token, { submissions, delivery: journal, acceptSubmissions: true, schedules,
    collectionUi: { environment: "isolated-acceptance", temporalUi: [{ clusterId: gateway.target.clusterId, baseUrl: "http://127.0.0.1:8235" }] },
  });
  const app = new Hono();
  app.use("*", async (c, next) => {
    if (c.req.header("host") !== "127.0.0.1:4185") return c.text("Local acceptance only", 403);
    c.header("Cache-Control", "no-store"); await next();
  });
  app.all("/api/v3/*", async c => {
    if (c.req.header("X-V3-Client") !== "local-workspace" ||
        (c.req.header("origin") && c.req.header("origin") !== "http://127.0.0.1:4185") ||
        (c.req.method !== "GET" && c.req.header("origin") !== "http://127.0.0.1:4185")) return c.text("Local acceptance only", 403);
    const req = new Request(c.req.raw); req.headers.set("authorization", `Bearer ${token}`);
    if (rejectNext && c.req.method === "POST" && c.req.path.endsWith("/schedule")) {
      rejectNext = false;
  ...
  server = serve({ fetch: app.fetch, hostname: "127.0.0.1", port: 4185 });
  ...
  const proof = async () => {
    ...
    const evidence = { audit, results, plan: await schedules.get(brand.id,source.id), tickReceipts: (await db.pool.query("SELECT request_id,result FROM api_request_receipt WHERE operation LIKE 'collection.schedule:%' ORDER BY created_at")).rows, active: await submissions.active(brand.id, source.id), address: env!.address, brand, source };
    await writeFile(join(db.root, "schedule-ui-proof.json"), JSON.stringify(evidence,null,2), { mode: 0o600 });
    event({ event: "PROOF_SAVED", path: join(db.root, "schedule-ui-proof.json"), submissions: rows.length });
  };
  input.on("line", command => { void (async () => {
    switch (command.trim()) {
      ...
      case "tick": await env!.client.schedule.getHandle(scheduleIdFor(source.id)).trigger("SKIP"); break;
  ...
  event({ event: "UI_READY", url: `http://127.0.0.1:4185/v3-live.html?brand=${brand.id}&scheduleSource=${source.id}#schedules`, temporalUi: "http://127.0.0.1:8235", evidence: db.root, pid: process.pid });
  ...
  for (const w of scheduleWorkers) w.shutdown();
  await Promise.allSettled(scheduleRuns);
