...
import { RegisteredOcrEvidence, RemoteOcrEvidence, KeywordPublication, VisionHandoff, VisionModule, PostgresVisionRegistry, CodexVisionProvider, visionReviewWriter, assertLabelVisionRegistrySchema } from "@crawl-automation/v3-vision";
import { PostgresReviews, RemoteReviews, ReviewsWithRemoteFallback, type PrivateReviewReader, type ReviewWriter } from "@crawl-automation/v3-review";
...
import {enrichmentStores} from "./enrichment-store.js";
import {ProductEnrichment} from "@crawl-automation/v3-product";
...
export const channelLabelRoutes:Record<string,string[]>={
 ...
 core:["prepareSwansonLabelCore"],assembly:["assembleLabelProduct"],collection:["collectLabelProduct","enrichProduct"],review:["reviewChannelProduct"],
...
export async function channelLabelRole(o:{role:string;hostId:string;root:string;remote:ObjectStore;db?:pg.Pool;resourceDb?:pg.Pool;storageId:string;
 ...
 // Cloud mode: no ledger. Only ocr/text/vision run; they retain evidence and Reviews remotely for the Mini receipt/consumer step.
 if(!o.db&&!["ocr","text","vision"].includes(o.role))throw Error("CHANNEL.DATABASE_REQUIRED");
 const {root,storageId}=o,db=o.db as pg.Pool,remote=new ActivityObjectReads(o.remote),local=await TextLocalStore.open(join(root,"journal"));
 const remoteReviews=new RemoteReviews(o.remote),ledgerReviews=o.db?new PostgresReviews(o.db):undefined;
 ...
 const registry=once("ocr-registry",async()=>o.db?new PostgresResultRegistry(o.db):null);
 const ledgerRegistry=async()=>{const r=await registry();if(!r)throw Error("LABEL.LEDGER_REQUIRED");return r;};
 ...
 // Cloud mode: OCR-sourced text and vision selections are verified against remote OCR evidence (no ledger).
 const remoteScreen=once("remote-ocr",async()=>new RemoteOcrEvidence(await artifacts(),await results(),o.remote));
 const textEvidence=once("text-evidence",async()=>new TextEvidence(await artifacts(),await results(),o.db?undefined:await results()));
 const textHandoff=once("text-handoff",async()=>new TextHandoff(local,remote,o.db?new PostgresTextRegistry(o.db):null,await textEvidence(),storageId));
 const visionHandoff=once("vision-handoff",async()=>new VisionHandoff(local,remote,o.db?new PostgresVisionRegistry(o.db):null,storageId,async(task,s)=>{if(o.db)await(await screen()).verifiedText(task.input.selection,s);else await(await remoteScreen()).verifiedText(task.input.selection,s);},{settleRemote:!!o.db}));
 const core=once("label-core",async()=>new PrepareSwansonLabelCore(await artifacts(),remote));
 const saved=once("saved-source",async()=>new SavedSourceEvidence({remote,files:await files(),pages:await pages(),reviews,ocr:await ledgerRegistry(),screen:await screen()}));
 ...
 case "vision":{
  ...
  activities.interpretImage=async(raw,signal)=>{const task=VisionTaskSchema.parse(raw);if(task.configFingerprint!==meta.configFingerprint)throw Error("VISION.CONFIG_MISMATCH");
    ...
    if(outcome.status!=="review"){if(outcome.replayed)outcome={...outcome,status:"review",code:"VISION.HANDOFF_PENDING"};else try{await h.complete(task,signal);return{status:o.db?"registered":"uploaded",operationId:task.input.operationId};}catch{outcome={...outcome,status:"review",code:"VISION.HANDOFF_PENDING"};}}
 ...
 case "collection":{const m=new CollectLabelProduct({local,remote,reviews,assembly:await assembly(),registry:new PostgresLabelCollectedProducts(db)}),history=historyObservations(db,remote);await history?.check();activities.collectLabelProduct=async(r,s)=>{const result=await m.run(r,s);if(result.status==="collected")await history?.attempt("collected",result.operationId,s);return result;};
  if(o.codex){const p=await CodexTextProvider.open(o.codex,environment);closers.push(()=>p.close());constructed.push("enrichment-provider");
   const e=new ProductEnrichment({...enrichmentStores(db),publication,reviews,provider:{provider:p.provider,interpret:(request,s)=>p.interpret(request,s)}});
   activities.enrichProduct=(r,s)=>e.run(r,s);}
  break;}
