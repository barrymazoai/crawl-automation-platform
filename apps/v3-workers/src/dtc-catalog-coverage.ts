...
import { verifyDtcCatalogCoverage } from '../../../packages/v3-channels/src/dtc-catalog-coverage.js';
...
export async function captureDtcCatalogCoverage(out:string,url:string,site:DtcSitePolicy,page:CdpOwnedPage,port:CdpTaskPort,signal:AbortSignal){
 ...
 if(root.pathname!=='/collections/all'||root.search||site.selectedUrls!==null||site.catalogPages.length!==1||site.catalogPages[0]!==url)return undefined;
 ...
 for(let index=1;index<=2;index++){
  const target=`${root.origin}/products.json?limit=100&page=${index}`;
  const response=await evaluate(`(async()=>{const c=new AbortController(),timer=setTimeout(()=>c.abort(),10000);try{const r=await fetch(${JSON.stringify(target)},{credentials:'include',signal:c.signal,headers:{accept:'application/json'}});const body=await r.text();if(body.length>524288)throw Error('catalog_response_limit');return {url:r.url,status:r.status,contentType:r.headers.get('content-type')||'',body};}finally{clearTimeout(timer);}})()`);
 ...
 const proof=DtcCatalogCoverageSchema.parse({version:'shopify-all-products/1',catalogUrl:url,dom,responses});
