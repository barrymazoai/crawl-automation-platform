...
export async function verifyDtcCaptureReview(job:DtcProductJob,raw:unknown,execution:{workflowId:string;runId:string},publication:RetainedPublication,s:AbortSignal){
 ...
 if(proof.operationId!==receipt.operationId||proof.url!==receipt.url||!equal(proof.execution,dtcExecutionIdentity(execution))||proof.closure.taskId!==job.sessionId||/user.?control|permission|sandbox|session_unavailable/i.test(proof.result.reasonCode??''))invalid();
