...
import { LocalVisionEvidenceStore, RegisteredOcrEvidence, KeywordPublication, keywordCompatibility, createKeywordRole, keywordReviewWriter } from "@crawl-automation/v3-vision";
...
const configSchema = z.strictObject({ storageId: VersionTagSchema,
  cacheRoot: z.string().refine(isAbsolute), ocrJournalRoot: z.string().refine(isAbsolute), keywordLocalRoot: z.string().refine(isAbsolute),
...
async function main() {
  if (process.env.V3_KEYWORD_LIVE_ENABLED !== "true" || !process.env.V3_KEYWORD_CONFIG || !isAbsolute(process.env.V3_KEYWORD_CONFIG))
    throw Error("Explicit keyword opt-in and private configuration required");
  const path = process.env.V3_KEYWORD_CONFIG, info = await lstat(path);
  ...
  const role = createKeywordRole({ buildId, compatibility: keywordCompatibility, testOnly: false,
    prepare: async (_workerConfig, signal) => {
      ...
      try {
        ...
        const local = await LocalVisionEvidenceStore.open(config.keywordLocalRoot);
        const publication = new KeywordPublication(local, r2.store);
        return { dispose, evidence, publish: (result, signal) => publication.publish(result, signal),
          recordReview: keywordReviewWriter(local, new PostgresReviews(reviewDb)) };
...
main().catch(() => {
  console.error(JSON.stringify({ event: "KEYWORD_WORKER_STARTUP_REJECTED", message: "Check opt-in, private config, dependency capabilities" }));
  process.exitCode = 1;
});
