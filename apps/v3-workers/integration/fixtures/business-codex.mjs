...
rl.on("line", async line => {
  ...
  if (method === "model/list") { emit({ id, result: { data: [{ id: "fixture", model: "fixture-model", inputModalities: ["text"], supportedReasoningEfforts: [{ reasoningEffort: "high" }] }], nextCursor: null } }); return; }
  ...
  if (method === "turn/start") {
    ...
    const evidence = JSON.parse(params.input[0].text.split("\n").at(-1));
    const itemText = evidence.lines[0].text.replace(/^Other ingredients: /, "");
    ...
    const raw = JSON.stringify({ formula: null, ingredients: { items: [{ quote: { text: itemText, fromLine: 1, toLine: 1 }, role: "other", parentNutrientIndex: null }] },
      excluded: [{ quote: { text: "Other ingredients:", fromLine: 1, toLine: 1 }, reason: "heading" }], issues: [] });
    setTimeout(() => {
