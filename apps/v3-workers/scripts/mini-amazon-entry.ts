...
async function main(){
 ...
 const u=new URL(url!);assert.ok(["http://127.0.0.1:4188","http://127.0.0.1:4189"].includes(u.origin));assert.equal(u.pathname,"/v3-live.html");
 const root="/Users/barry/apps/crawlv3-batch-a.UiA4dx",dir=root+"/live/amazon-entry-20260911/"+name;
 await mkdir(root+"/live/amazon-entry-20260911",{recursive:true,mode:0o700});await mkdir(dir,{mode:0o700});
 ...
 if(mode==="submit"){
  assert.equal(u.origin,"http://127.0.0.1:4189");
  assert.equal(u.searchParams.get("brand"),"525432fc-e055-45f9-a7ed-073602e16209");assert.equal(u.searchParams.has("request"),false);
  const status=await read(root+'/release-amazon-mainflow-20260911-v2/status.json');assert.equal(status.jobs.filter((j:any)=>j.ready).length,30);assert.ok(status.dependencies.every((d:any)=>d.healthy));
 ...
 try{
  ...
  await pages.using(id,AbortSignal.timeout(120000),async page=>{
   ...
   const focus=u.searchParams.has("request")?`await js("document.querySelector('#collection')?.scrollIntoView({block:'start'})");await new Promise(r=>setTimeout(r,300));`:"";
   ...
   if(mode==="submit"){
    assert.match(initial.value.body,/UNIQUE E/i);assert.match(initial.value.body,/amazon/);
    ...
    const ready=`(()=>{const b=[...document.querySelectorAll('button')].filter(b=>b.innerText.trim()==='提交一次采集');return b.length===1&&!b[0].disabled&&document.querySelector('#collection')?.innerText.includes('amazon');})()`;
