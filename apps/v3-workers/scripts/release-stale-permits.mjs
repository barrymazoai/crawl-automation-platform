...
try{
 ...
 for(const row of held){const {workflowId,runId}=row.request;const handle=client.workflow.getHandle(workflowId,runId);const d=await handle.describe();
  ...
  // An Activity left open when the workflow closed was abandoned by Temporal; the permit only blocks settlement now.
  const openActivities=scheduled-ended;const closedAgo=d.closeTime?Date.now()-new Date(d.closeTime).getTime():0;if(openActivities&&closedAgo<120000){console.log(JSON.stringify({event:'PERMIT_KEPT',permitId:row.permit_id,workflowId,reason:'closed <2min ago with open activities',open:openActivities}));continue;}
