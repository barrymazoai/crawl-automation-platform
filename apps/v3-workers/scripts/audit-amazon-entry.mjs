...
const root='/Users/barry/apps/crawlv3-batch-a.UiA4dx',out=root+'/live/amazon-entry-20260911/request-'+request,read=async p=>JSON.parse(await fs.readFile(p));await fs.mkdir(out,{recursive:true,mode:0o700});
...
const config=await read(root+'/live/amazon-mainflow-20260911/release-amazon-mainflow-20260911-v2/amazon.private.json'),runtime=await read(root+'/live/amazon-mainflow-20260911/release-amazon-mainflow-20260911-v2/amazon-brand-workflow.runtime.json'),t=runtime.transport;
...
try{
 ...
 while(queue.length){const id=queue.shift();if(seen.has(id))continue;seen.add(id);const h=client.workflow.getHandle(id),d=await h.describe(),history=await h.fetchHistory();
  for(const e of history.events??[]){const child=e.childWorkflowExecutionStartedEventAttributes?.workflowExecution?.workflowId;if(child)queue.push(child);
   ...
   if(job?.codec==='amazon-product-job/1')taskIds.add(job.sessionId);
   if(e.activityTaskScheduledEventAttributes?.activityType?.name==='readCatalogPage'){const input=decode(e.activityTaskScheduledEventAttributes.input?.payloads?.[0]);taskIds.add('amazon-catalog-'+createHash('sha256').update(JSON.stringify(input)).digest('hex'));}
  }
  ...
  if(final){assert.equal(d.status.name,'COMPLETED');await Worker.runReplayHistory({workflowBundle:{codePath:root+'/release-amazon-mainflow-20260911-v2/product-workflows.cjs'}},history,id);item.replay=true;await fs.writeFile(out+'/history-'+d.runId+'.json',JSON.stringify(history));}
  workflows.push(item);
 }
 const submission=(await db.query('SELECT snapshot FROM collection_submission WHERE request_id=$1',[request])).rows[0],rows={};assert.equal(submission.snapshot.channel,'amazon');assert.equal(submission.snapshot.sourceRevision,2);
 ...
 if(final){
  assert.equal(report.held,0);assert.equal(workflows.filter(w=>w.type==='AmazonCatalogProductWorkflow').length,1);
  ...
  report.pages=[];for(const name of (await fs.readdir(config.pageJournalRoot,{recursive:true})).filter(n=>n.endsWith('/opened.json'))){const opened=await read(config.pageJournalRoot+'/'+name);if(!taskIds.has(opened.taskId))continue;const closed=await read(config.pageJournalRoot+'/'+name.replace('/opened.json','/closed.json'));assert.deepEqual(closed,opened);report.pages.push({taskId:opened.taskId,targetId:opened.targetId,closed:true});}assert.equal(report.pages.length,2);assert.equal(report.globalHeld,0);assert.equal(report.guards,0);
