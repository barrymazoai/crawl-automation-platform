...
const tests=await read(work+'/test-results.json');assert.equal(tests.numFailedTests,0);assert.equal(tests.numPendingTests,0);assert.ok(tests.numPassedTests>=75);assert.equal((await read(work+'/replay-results.json')).passed,true);
...
try{
 ...
 const priorIds=[...plan.batches.map(b=>b.requestId),(await read(previous+'/label-header-retry/started.json')).requestId,(await read('/Users/barry/apps/crawlv3-history-20260913/amazon-throughput-20260914/started.json')).requestId];
 ...
 const releases={},releaseRoot=main+'/release-amazon-detached-files-20260914';await fs.mkdir(work+'/runtime',{mode:0o700});
 for(const group of ['amazon','workflow']){
  const dest=releaseRoot+'/'+group;await fs.mkdir(dest,{recursive:true,mode:0o700});const hash=createHash('sha256');
  for(const name of (await fs.readdir(work+'/candidate/'+group)).filter(n=>n.endsWith('.js')||n==='product-workflows.cjs').sort()){
 ...
 const roles=[...['control','catalog-source','catalog-ledger','product-input','file','review','capture'].map(r=>['amazon-'+r,'amazon']),['amazon-product-workflow','workflow']];
 const entriesByGroup={amazon:'amazon-live-worker.js',workflow:'product-workflow-worker.js'},restarted=[];
 ...
 for(const [id,group] of roles){
  ...
  await keep(job.env.V3_WORKER_CONFIG,{...r,expectedBuildId:release.buildId,...(id==='amazon-file'?{concurrency:2}:{})});
  ...
  assert.equal(await fs.readFile(manifest,'utf8'),saved);saved=JSON.stringify(m,null,2);await keep(manifest+'.detached-next',saved);await fs.rename(manifest+'.detached-next',manifest);
  ...
  const q=await connection.workflowService.describeTaskQueue({namespace:runtime.namespace,taskQueue:{name:h.taskQueue},taskQueueType:group==='workflow'?1:2});assert.ok(q.pollers?.some(p=>p.identity===h.identity));
  restarted.push({id,pid:h.pid,buildId:h.buildId});await keep(work+'/restarted-'+id+'.json',restarted.at(-1));console.log(JSON.stringify({event:'DETACHED_WORKER_READY',...restarted.at(-1)}));
 }
 ...
 await idle();await keep(work+'/deployment.json',{at:new Date().toISOString(),passed:true,restarted,unrelatedPidsUnchanged:true,amazonProductWorkflowUpdated:true,capacity});
 ...
 await keep(work+'/started.json',started);console.log(JSON.stringify({event:'DETACHED_TEN_SUBMITTED',...started}));
