/** Upgrade the existing Swanson activity roles and its catalog/product workflow jobs after replay and drain. No new submission or business retry. */
...
const root='/Users/barry/apps/crawlv3-batch-a.UiA4dx',resident=root+'/live/channel-resident-20260910-v2',dir=root+'/live/swanson-coverage-deployment-20260910',release=root+'/release-swanson-coverage-20260910',read=async p=>JSON.parse(await fs.readFile(p,'utf8'));
...
const tests=await read(root+'/swanson-coverage-tests-20260910-final-v2/tests.json');assert.equal(tests.success,true);assert.equal(tests.numTotalTests,81);assert.equal(tests.numFailedTests,0);
const proof=await read(root+'/swanson-coverage-tests-20260910-final-v2/channel-stream-proof.json');assert.equal(proof.passed,true);assert.equal(proof.replays,6);
const familyProof=await read(root+'/swanson-coverage-tests-20260910-final-v2/family-proof.json');assert.equal(familyProof.passed,true);assert.equal(familyProof.replays,4);
const ledgerProof=await read(root+'/swanson-coverage-tests-20260910-final-v2/catalog-ledger-proof.json');assert.equal(ledgerProof.passed,true);
...
if(mode==='--prepare'){
 ...
 await fs.cp(root+'/staging-swanson-coverage-20260910-v2',release,{recursive:true,errorOnExist:true,force:false});
 ...
 const manifest=structuredClone(previous),jobs=[];const config=await read(resident+'/swanson.private.json');config.maxPages=10;await fs.writeFile(dir+'/swanson.private.json',JSON.stringify(config),{mode:0o600,flag:'wx'});for(const j of manifest.jobs.filter(j=>['swanson-control','swanson-catalog-source','swanson-catalog-ledger','swanson-product-input','swanson-capture','swanson-file','swanson-review','swanson-product-workflow','swanson-catalog-workflow'].includes(j.id))){const r=await read(j.env.V3_WORKER_CONFIG),workflow=j.entry.endsWith('/product-workflow-worker.js');r.expectedBuildId=workflow?buildId:activityBuild;j.entry=release+'/'+(workflow?'product-workflow-worker.js':'swanson-live-worker.js');j.env.V3_WORKER_CONFIG=dir+'/'+j.id+'.runtime.json';await fs.writeFile(j.env.V3_WORKER_CONFIG,JSON.stringify(r),{mode:0o600,flag:'wx'});if(j.env.V3_SWANSON_LIVE_CONFIG)j.env.V3_SWANSON_LIVE_CONFIG=dir+'/swanson.private.json';jobs.push(j.id);}assert.equal(jobs.length,9);
...
try{
 const client=new Client({connection,namespace:runtime.namespace}),results=[],active=[],types=new Set(['GncLeasedProductWorkflow','CatalogWorkflow','CatalogProductWorkflow','BrandCollectionWorkflow','ScheduleIntakeWorkflow','SwansonCatalogProductWorkflow','ChannelSavedLabelWorkflow','ChannelStreamingLabelWorkflow','SwansonVariantProductWorkflow']);
 ...
 if(mode==='--prepare'){console.log(JSON.stringify({prepared:true,jobs:ready.jobs,replays:results.length,active:active.length}));process.exitCode=0;}
  ...
  const live=await read(dir+'/evidence/live-proof.json');assert.equal(live.browser.status,'passed');assert.equal(live.label.status,'passed');assert.equal(live.label.counts.collected_product,2);assert.equal(live.label.held,0);
  const preflight=await read(dir+'/evidence/preflight.json');assert.equal(preflight.passed,true);assert.equal(preflight.results.length,9);for(const job of ready.jobs)assert.ok(preflight.results.some(r=>r.role===job&&r.ready&&r.normalExit));
