...
const out = 'dist/cloud-workers', common = { config: false as const, format: 'esm' as const, noExternal: [/^@crawl-automation\/v3-/], external: [/^@temporalio\//, 'zod', 'pg', '@aws-sdk/client-s3', 'linkedom'] };
...
const require = createRequire(import.meta.url), channels = createRequire(new URL('../../../packages/v3-channels/package.json', import.meta.url));
const installed = (name: string) => JSON.parse(require('node:fs').readFileSync((name === 'linkedom' ? channels : require).resolve(name + '/package.json'), 'utf8')).version as string;
await writeFile(out + '/package.json', JSON.stringify({ name: 'crawlv3-cloud-workers', version: '0.1.0', private: true, type: 'module', engines: { node: '>=22.16 <25' },
  dependencies: Object.fromEntries(['@temporalio/worker', '@temporalio/client', '@temporalio/activity', '@temporalio/common', '@temporalio/workflow', 'pg', 'zod', '@aws-sdk/client-s3', 'linkedom'].map(n => [n, installed(n)])) }, null, 2) + '\n');
