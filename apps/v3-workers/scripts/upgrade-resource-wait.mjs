/** Upgrade only existing workflow jobs after replay and drain. No new submission or business retry. */
...
const root='/Users/barry/apps/crawlv3-batch-a.UiA4dx',resident=root+'/live/channel-resident-20260910-v2',dir=root+'/live/resource-wait-20260910',release=root+'/release-resource-wait-20260910',read=async p=>JSON.parse(await fs.readFile(p,'utf8'));
...
const tests=await read(resident+'/evidence/resource-wait-tests.json');assert.equal(tests.success,true);assert.equal(tests.numTotalTests,31);assert.equal(tests.numFailedTests,0);
const proof=await read(root+'/resource-wait-tests-20260910/resource-wait-proof.json');assert.equal(proof.passed,true);assert.equal(proof.syntheticBusinessCalls,1);
...
if(mode==='--prepare'){
 ...
 await fs.cp(root+'/release-channel-resident-20260910-v2',release,{recursive:true,errorOnExist:true,force:false});
 await fs.copyFile(root+'/resource-wait-tests-20260910/product-workflows.cjs',release+'/product-workflows.cjs');
 const names=(await fs.readdir(release)).filter(n=>n.endsWith('.js')||n==='product-workflows.cjs').sort(),hash=createHash('sha256');
 for(const n of names){const bytes=await fs.readFile(release+'/'+n);hash.update(String(bytes.length));hash.update(':');hash.update(bytes);}const buildId=hash.digest('hex');
 const manifest=structuredClone(previous),jobs=[];for(const j of manifest.jobs.filter(j=>j.entry.endsWith('/product-workflow-worker.js'))){const r=await read(j.env.V3_WORKER_CONFIG);r.expectedBuildId=buildId;j.entry=release+'/product-workflow-worker.js';j.env.V3_WORKER_CONFIG=dir+'/'+j.id+'.runtime.json';await fs.writeFile(j.env.V3_WORKER_CONFIG,JSON.stringify(r),{mode:0o600,flag:'wx'});jobs.push(j.id);}
...
try{
 const client=new Client({connection,namespace:runtime.namespace}),results=[],active=[],types=new Set(['GncLeasedProductWorkflow','CatalogWorkflow','CatalogProductWorkflow','BrandCollectionWorkflow','ScheduleIntakeWorkflow','SwansonCatalogProductWorkflow','ChannelSavedLabelWorkflow']);
