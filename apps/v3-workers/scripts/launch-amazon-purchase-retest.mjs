...
const dir=history+'/amazon-purchase-retest-10-20260914',service=main+'/amazon-purchase-retest-10-20260914',campaignId='amazon-purchase-retest-10-us-10001-20260914';
...
try{
 ...
 const deployed=await read(history+'/purchase-conditions-20260914/deployment.json');assert.equal(deployed.passed,true);
 ...
 const batches=selected.map(b=>({...b,requestId:randomUUID()}));
 ...
 await keep(dir+'/selection-summary.json',{at:new Date().toISOString(),campaignId,productCount:10,priorCampaignId:prior.campaignId,priorCursor:24,policy:'First ten previously submitted products, fresh request and observation identities; retain all prior observations',products:products.map((p,i)=>({asin:p.asin,url:p.url,priorRequestId:oldIds[i],requestId:batches[i].requestId})),manifestSha256});
 ...
 const scope='amazon-purchase-retest-10-20260914',controlQueue='v3.amazon.batch.control.v1.amazon-batch-v1.session.'+scope;
 ...
 const handle=await client.workflow.start('AmazonHistoryBatchWorkflow',{workflowId:campaignId,taskQueue:workflowQueue,args:[{campaignId,manifestSha256,controlQueue,stopAfter:10}],workflowIdReusePolicy:'REJECT_DUPLICATE'});
 const d=await handle.describe();assert.equal(d.status.name,'RUNNING');
 const receipt={at:new Date().toISOString(),campaignId,runId:d.runId,productCount:10,chunks:10,stopAfter:10,manifestSha256,workflowQueue,controlQueue,controlPid:ch.pid,ready,oldCampaignPausedAt:24,oldCampaignWorkersUnchanged:true,unrelatedPidsUnchanged:true,businessReleasesUnchanged:true,region:'US',postalCode:'10001'};
