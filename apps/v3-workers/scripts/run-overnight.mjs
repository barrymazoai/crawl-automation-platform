...
const unrecordedMode=process.argv[2]==='retry-unrecorded',retryMode=process.argv[2]==='retry-capture'||unrecordedMode,retrySince=retryMode?process.argv[3]:null,total=retryMode?2000:Number(process.argv[2]),perRequest=Number(process.argv[retryMode?4:3]??10),maxConcurrent=Number(process.argv[retryMode?5:4]??8);assert.ok(total>=1&&total<=40000&&perRequest>=1&&perRequest<=10&&maxConcurrent>=1&&maxConcurrent<=2000);if(retryMode)assert.match(retrySince??'',/^\d{4}-\d{2}-\d{2}T/,'retry-capture needs an ISO start time');
...
// A restart can collide with one still settling from the previous run (2026-09-17: the next run started 7s after the
// last one restored and its first restart was refused). Retry with a pause instead of abandoning the whole run.
const restartOnce=async id=>{const r=await run('/opt/homebrew/bin/node',[controller,'restart',manifestPath,id],{timeout:180000,maxBuffer:1048576});assert.equal(JSON.parse(r.stdout).ready[0].id,id);const h=await read(main+'/'+id+'.health.json');assert.equal(h.event,'WORKER_RUNNING');};
const restart=async id=>{for(let attempt=1;;attempt++){try{return await restartOnce(id);}catch(e){if(attempt>=5)throw e;console.error(JSON.stringify({event:'RESTART_RETRY',id,attempt,message:String(e?.stdout??e?.message).slice(0,200)}));await pause(30000*attempt);}}};
...
try{
 ...
 let asins;
 // ASINS=a,b,c runs exactly those plan listings (canary / failure-path checks), regardless of earlier attempts.
 if(process.env.ASINS){const want=process.env.ASINS.split(',');asins=want.filter(a=>plan.products.some(p=>p.asin===a));assert.equal(asins.length,want.length,'ASINS not all in plan');}
 else if(unrecordedMode){const rows=(await q("SELECT DISTINCT d.record->'entry'->>'listingId' asin FROM catalog_discovery d WHERE d.record->'scope'->>'channel'='amazon' AND NOT EXISTS (SELECT 1 FROM collected_product c WHERE c.record->'observation'->>'listingId'=d.record->'entry'->>'listingId') AND NOT EXISTS (SELECT 1 FROM review_record r WHERE r.record->'observation'->>'listingId'=d.record->'entry'->>'listingId')")).rows;asins=rows.map(r=>r.asin).filter(a=>plan.products.some(p=>p.asin===a)).slice(0,total);}
 else if(retryMode){const rows=(await q("SELECT DISTINCT record->'observation'->>'listingId' asin FROM review_record WHERE registered_at>$1 AND record->'failure'->>'code'=ANY($2) AND coalesce(record->'rawError'->'details'->>'causeCode','') NOT IN ('AMAZON.NOT_FOUND','AMAZON.ASIN_CONFLICT') AND record->'observation'->>'listingId' NOT IN (SELECT record->'observation'->>'listingId' FROM collected_product WHERE collected_at>$1)",[retrySince,CAPTURE_REVIEW_CODES])).rows;asins=rows.map(r=>r.asin).filter(a=>plan.products.some(p=>p.asin===a)).slice(0,total);}
 else asins=plan.products.filter(p=>!attempted.has(p.asin)).slice(0,total).map(p=>p.asin);
 assert.ok(asins.length>=1,'nothing left to attempt');await log('OVERNIGHT_SELECTION',{mode:process.env.ASINS?'explicit':unrecordedMode?'retry-unrecorded':retryMode?'retry-capture':'never-attempted',asins:asins.length});
 ...
 await fs.mkdir(dir,{mode:0o700});await keep(dir+'/batches.json',batches);// Carry forward only link batches whose request is still running. Every past run's batches used to accumulate here
 // until the config passed the workers' 4 MB private-config limit and the Amazon roles refused to start (2026-09-17).
 const liveRoots=new Set();for await(const w of client.workflow.list({query:"WorkflowType = 'BrandCollectionWorkflow' AND ExecutionStatus = 'Running'"}))liveRoots.add(w.workflowId.replace(/^v3-collection-/,''));
 const carried=(live.linkBatches??[]).filter(b=>liveRoots.has(b.requestId));const nextConfig={...live,linkBatches:[...carried,...batches]};
 const configBytes=Buffer.byteLength(JSON.stringify(nextConfig,null,2));await log('OVERNIGHT_CONFIG',{carriedBatches:carried.length,droppedBatches:(live.linkBatches??[]).length-carried.length,newBatches:batches.length,bytes:configBytes});
 assert.ok(configBytes<3.8*1024*1024&&nextConfig.linkBatches.length<=2000,'Amazon config would exceed worker limits ('+configBytes+' bytes, '+nextConfig.linkBatches.length+' batches); run fewer products per launch');
 await keep(dir+'/amazon.private.json',nextConfig);await keep(dir+'/deployment-before.private.json',text);
 ...
 const finish=async(rid,status)=>{const {batch,started,n}=active.get(rid);active.delete(rid);
  ...
  const recorded=new Set((await q("SELECT record->'observation'->>'listingId' l FROM collected_product WHERE record->'observation'->>'requestId'=$1 UNION SELECT record->'observation'->>'listingId' FROM review_record WHERE record->'observation'->>'requestId'=$1",[rid])).rows.map(r=>r.l));const unrecorded=batch.entries.map(e=>e.entry.listingId).filter(a=>!recorded.has(a));const skipped=batch.entries.length-collected-reviews;const rec={n,requestId:rid,status,seconds:Math.round((Date.now()-started)/1000),products:batch.entries.length,collected,reviews,skipped:Math.max(0,skipped),enriched,unrecorded};
 ...
 while(next<batches.length||active.size){
  ...
  for(const [rid,{started}] of [...active.entries()]){let d;try{d=await client.workflow.getHandle('v3-collection-'+rid).describe();}catch(e){if(e?.name==='WorkflowNotFoundError'&&Date.now()-started<15*60000)continue;await log('REQUEST_LOST',{requestId:rid,message:String(e?.message).slice(0,120)});await finish(rid,'LOST');continue;}
   if(d.status.name!=='RUNNING'){await finish(rid,d.status.name);continue;}
   // A request's slot is freed as soon as every one of its products has stopped, whatever the outcome. A product that
   // FAILED without a terminal record would otherwise keep the root polling and the slot occupied for hours
   // (2026-09-17). Such products are logged for retry and the idle root is terminated.
   if(Date.now()-started>8*60000){try{const cat=await client.workflow.getHandle('v3-collection-'+rid+'-catalog').describe();
    if(cat.status.name!=='RUNNING'){let running=0,total=0;const failed=[];for await(const c of client.workflow.list({query:`ParentWorkflowId = 'v3-collection-${rid}-catalog'`})){total++;if(c.status.name==='RUNNING')running++;else if(c.status.name!=='COMPLETED')failed.push({workflowId:c.workflowId,status:c.status.name});}
     if(total>0&&running===0){await log('REQUEST_PRODUCTS_SETTLED',{requestId:rid,products:total,failed});
      if(failed.length){try{await client.workflow.getHandle('v3-collection-'+rid).terminate('runner: every product stopped; '+failed.length+' failed without a terminal record');}catch(e){await log('ROOT_TERMINATE_FAILED',{requestId:rid,message:String(e?.message).slice(0,120)});}}
      await finish(rid,failed.length?'PRODUCTS_SETTLED_WITH_FAILURES':'PRODUCTS_SETTLED');continue;}}}catch(e){if(e?.name!=='WorkflowNotFoundError')await log('SETTLE_CHECK_FAILED',{requestId:rid,message:String(e?.message).slice(0,120)});}}
   if(Date.now()-started>3*3600000){await log('REQUEST_ABANDONED',{requestId:rid,hours:3});await finish(rid,'ABANDONED');}}
