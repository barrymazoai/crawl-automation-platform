...
const c = JSON.parse(await fs.readFile(dir + "/swanson.private.json", "utf8"));
...
try {
  ...
  const workflowIds = [`v3-collection-${requestId}`, `v3-collection-${requestId}-catalog`, ...discoveries.flatMap(d => [d.workflowId, d.workflowId + "-label"])];
  const evidence = [];
  ...
  for (const table of ["collected_product", "review_record", "processing_result"])
    rows[table] = (await db.query(`SELECT record_hash, record FROM ${table} WHERE record::text LIKE $1`, ["%" + requestId + "%"])).rows;
  const workflows = [];
  ...
  for (const id of workflowIds) {
    ...
    if (mode) {
      ...
      await fs.writeFile(dir + `/evidence/history-${d.runId}.json`, JSON.stringify(history), { mode: 0o600 });
  ...
  if (mode) {
    ...
    try {
      for (const ref of refs.values()) {
        ...
        if (ref.kind === "source-image") {
          ...
          if (extension) await fs.writeFile(dir + `/evidence/source-${ref.sha256}.${extension}`, bytes, { mode: 0o600 });
        }
      }
      report.modelStopProofs = [];
      for (const row of rows.review_record) {
        const review = row.record;
        if (!/^(TEXT|VISION)\.LABEL_/.test(review.failure.code)) continue;
        const workflow = workflows.find(w => w.type === "ChannelSavedLabelWorkflow" && w.result?.observationId === review.observation.observationId);
        assert.ok(workflow);
    ...
    const before = JSON.parse(await fs.readFile(root + "/live/swanson-mainflow-20260910/configuration-v2/before-run-records.json", "utf8"));
    for (const [table, values] of Object.entries(before)) {
      ...
      for (const row of values) assert.ok(now.has(row.record_hash));
    }
    report.oldRecordsPreserved = true;
    await fs.writeFile(dir + "/evidence/records.json", JSON.stringify({ discoveries, evidence, rows }), { mode: 0o600 });
  }
  await fs.writeFile(dir + "/evidence/report.json", JSON.stringify(report, null, 2), { mode: 0o600 });
