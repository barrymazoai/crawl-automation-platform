...
const groupRoot=group=>group==='batch'?pilot+'/release-resource-stall-20260914':release+'/'+group;
...
try{
 ...
 for(const group of ['label','workflow','amazon','batch','recovery']){
  const source=dir+(group==='batch'?'/amazon-batch':'/resource-stall/'+group),target=groupRoot(group);await fs.mkdir(target,{mode:0o700});
 ...
 for(const [group,ids] of Object.entries(scopes))for(const id of ids){const index=group==='batch'?1:0,j=after[index].jobs.find(j=>j.id===id);assert.ok(j);const r=await read(j.env.V3_WORKER_CONFIG);
  j.entry=groupRoot(group)+'/'+entries[group];j.env.V3_WORKER_CONFIG=dir+'/runtime/'+id+'.json';await keep(j.env.V3_WORKER_CONFIG,{...r,expectedBuildId:builds[group]});
 ...
 for(const [group,roles] of Object.entries(scopes))for(const id of roles){
  ...
  assert.ok((await exec('/bin/ps',['-p',String(health.pid),'-o','command='])).stdout.includes(groupRoot(group)+'/'+entries[group]));
