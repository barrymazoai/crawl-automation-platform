#!/bin/zsh
# 整合 / Unify
# 对应入口：apps/backend/src/workers/unify.ts
set -euo pipefail
...
set +a
export NODE_ID="${NODE_ID:-$(hostname -s)-unify}"
...
[[ -f apps/backend/dist/workers/unify.js ]] || pnpm build
exec pnpm --filter @crawl-automation/backend worker:unify
