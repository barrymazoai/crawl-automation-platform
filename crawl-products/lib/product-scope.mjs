const BUNDLE_TITLE_RE =
  /\b(?:bundle|kit|stack|collection|regimen|set|starter\s+set|gift\s+set|value\s+pack|variety\s+pack|family\s+pack|multi[-\s]?pack|pack|duo|trio)\b/i;
...
const DECISIVE_NON_NUTRITION_CONTEXT_RE =
  /\b(?:green\s+home|household|home\s+care|cleaning|laundry|dishwashing|pet|equine|veterinary)\b/i;
...
export function classifyNutritionSingleProduct(record) {
  ...
  const ingredientEvidence = hasValue(
    fields.main_ingredients
    ?? fields.mainIngredients
