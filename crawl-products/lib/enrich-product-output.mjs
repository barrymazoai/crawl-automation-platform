...
import { classifyNutritionSingleProduct } from "./product-scope.mjs";
import {
...
/** Read the engine-owned state.json value, or null if absent/unreadable. */
async function readStateValue(outDir) {
  try {
    const raw = await fs.readFile(path.join(outDir, "state.json"), "utf8");
...
function assertApiReadyInput(input, record, options = {}) {
  ...
  const fields = record.fields && typeof record.fields === "object" ? record.fields : record;
  // HTTP-sourced records may omit browser-only Facts/gallery review, but they
  // never get an ingredient exemption. product/enrich has no separate formula
  // write path, so formula rows must be materialized into mainIngredients first.
...
export function buildEnrichProductExport(records, options = {}) {
  ...
  for (const [index, record] of (Array.isArray(records) ? records : []).entries()) {
    const scopeDecision = classifyNutritionSingleProduct(record);
    if (!scopeDecision.included) {
      exclusions.push({
        index,
        productName: scopeDecision.title,
        sourceUrl: scopeDecision.url,
        reason: scopeDecision.reason,
        evidence: scopeDecision.evidence,
      });
      continue;
    }
    try {
      ...
      if (representative.mainIngredients.length === 0) {
        exclusions.push({
          index,
          productName: representative.productName || null,
          sourceUrl: representative.sourceUrl || null,
          reason: "ingredients_and_formula_missing",
          evidence: "api_ready_payload",
        });
        continue;
      }
  ...
  return {
    ...
    summary: {
      ...
      recordsMapped,
      recordsExcluded: exclusions.length,
      exclusionReasons: exclusions.reduce((counts, exclusion) => {
...
export async function writeEnrichProductExport(outDir, records, options = {}) {
  ...
  if (output.summary.outputMode === "inventory_partial") {
    ...
    const files = {
      ...
      exclusions: path.join(outDir, "product-enrich-exclusions.json"),
    ...
    await Promise.all([
      ...
      writeAtomic(files.exclusions, `${JSON.stringify(output.exclusions, null, 2)}\n`),
  ...
  const onDiskState = await readStateValue(outDir);
  const onDiskStateAllowsFormal = onDiskState == null
    || ["complete", "verified"].includes(onDiskState);
  const formalArtifactsReady = output.summary.completionStatus === "complete"
    && runCompletionStatus === "complete"
    && onDiskStateAllowsFormal;
  if (!formalArtifactsReady) {
    ...
    const summary = {
      ...
      blockingReasons: [
        ...
        ...(onDiskStateAllowsFormal ? [] : [`state_not_complete:${onDiskState}`]),
