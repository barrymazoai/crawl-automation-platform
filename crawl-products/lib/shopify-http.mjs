 ...
 */
/**
 * Best-effort ingredient extraction from a Shopify body_html. Many stores put
 * the Supplement Facts / ingredient list in the product description (~half do,
 * highly variable), so slice out the section that starts at an ingredient/facts
 * heading. Returns "" when no such section exists — the semantic phase then
 * falls back to reading Facts images, and if there are none, accepts empty
 * (best-effort, never blocks). Multi-language headings included.
 */
...
export function extractIngredientsFromBody(bodyHtml) {
  ...
  if (!m) return "";
  // from the heading to the end (or to a clear next-section cutoff)
  const slice = text.slice(m.index);
  const cut = slice.search(/\b(directions?|modo d.uso|suggested use|warnings?|disclaimer|reviews?)\b/i);
  ...
  return {
    ...
    hooks: {
      ...
      fetchPageHtml: async (url) => {
          ...
          if (!response.ok || !/html/i.test(response.headers.get("content-type") || "")) throw new Error(`html_http_invalid:${response.status}`);
