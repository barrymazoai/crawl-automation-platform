...
默认是严格导出：先用 `classifyNutritionSingleProduct()` 复判范围，非 Nutrition 记录直接排除；通过范围判断但没有可入库 `mainIngredients` 的记录按 `ingredients_and_formula_missing` 排除。当前 `product/enrich` 不单独写 formula，结构化 formula/Facts 必须先物化为 `mainIngredients`。其余缺真实 `productUrl`、至少一张图片、`galleryReview.status:"visual_complete"`、DOM+画廊 Facts source review、`productForm`、`healthFunctions`、对应语义证据，或存在缺 `substance/category` 的主成分时，记录进入错误文件。有多张 Facts 图时必须逐张完成成分复核。
...
- `product-enrich-exclusions.json`：最终范围/数据充分性过滤掉的记录及原因；不进入请求，也不阻塞同批合法产品。
...
1. `recordsMapped + recordsExcluded + errors === recordsReceived`；`inputsReady` 是展开后的变体请求数，允许大于 `recordsMapped`。
