...
Nutrition 范围判断通过后还要过一层数据充分性检查：记录必须已经有可入库的
`mainIngredients`，或先把结构化 formula/Facts 行解析并物化成 `mainIngredients`。
当前 `product/enrich` 不单独写 formula，因此不能先导入一个空产品再等待后续补齐。

完整检查详情正文、标签图和 Facts 图后仍没有 ingredients/formula 的记录，按
`ingredients_and_formula_missing` 排除并保留证据；它不是 review，也不得出现在
API-ready 请求里。该规则同样适用于 Shopify HTTP 快速通道。
