...
- 最终只入库已确认的 Nutrition 单品。非营养商品在语义阶段标记 `excluded`，导出器还会独立复判；没有可入库 `mainIngredients` 的记录按 `ingredients_and_formula_missing` 跳过。当前 `product/enrich` 没有独立 formula 写入路径，因此标签 formula 必须先解析并物化成 `mainIngredients`，不能以“后续再补”为由先建空产品。
...
- **HTTP 通道不豁免成分门**：`products.json` 没有结构化 Facts，所以语义阶段必须从详情正文和已下载商品图中读取并物化 `main_ingredients`。完整检查后仍没有成分/配方的记录直接标记 `excluded:ingredients_and_formula_missing`，不进入 review，也不进入 API-ready；不能因为 HTTP 通道快就先建空产品。HTTP 通道只可放宽浏览器专属的 Facts/gallery 复核形式，不能放宽 Nutrition 终判或成分存在性。
...
最终过滤会把非 Nutrition 与没有 ingredients/formula 的记录写入 `product-enrich-exclusions.json` 并从请求中跳过；这类已解释的排除不阻塞同批合法产品。其他校验错误仍保持批次级全有或全无。
