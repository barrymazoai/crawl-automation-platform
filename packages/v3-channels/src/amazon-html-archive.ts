...
export class AmazonHtmlArchive {
  ...
  async save(bytes: Uint8Array, signal: AbortSignal, metadata: { capturedAt?: string; fetchedVia?: AmazonRenderedProduct['fetchedVia'] } = {}): Promise<ArchivedAmazonHtml> {
    ...
    const receipt = receiptSchema.parse({ codec: 'amazon-original-html/1', operationId: this.job.operationId,
      ...
      ...(metadata.fetchedVia ? { fetchedVia: metadata.fetchedVia } : {}) });
