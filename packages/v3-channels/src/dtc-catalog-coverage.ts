...
/** A category must prove its own exhaustion, never the unrelated store-wide set. */
export function dtcCatalogCoverageTarget(url:string){
 ...
 return match[1]==='all'
  ?{version:'shopify-all-products/1' as const,endpoint:root.origin+'/products.json'}
  :{version:'shopify-collection-products/1' as const,endpoint:root.origin+'/collections/'+match[1]+'/products.json'};
}
...
export function verifyDtcCatalogCoverage(raw:unknown,url:string,entries:{url:string}[]):boolean{
 const proof=DtcCatalogCoverageSchema.parse(raw),root=new URL(url),target=dtcCatalogCoverageTarget(url);
 if(!target||proof.version!==target.version||proof.catalogUrl!==url||proof.dom.url!==url)throw Error('DTC.CATALOG_END_UNVERIFIED');
 ...
 for(const [index,response] of proof.responses.entries()){
  if(terminal||response.url!==`${target.endpoint}?limit=100&page=${index+1}`||!/json/i.test(response.contentType))throw Error('DTC.CATALOG_END_UNVERIFIED');
