...
vi.mock("@temporalio/workflow",()=>({patched:()=>true,proxyActivities:()=>({prepareBrandCollection:env.prepare,inspectBrandCollection:env.inspect}),workflowInfo:()=>({workflowId:`v3-collection-${env.id}`,taskQueue:"brand"}),startChild:env.start,sleep:env.sleep,ParentClosePolicy:{ABANDON:"ABANDON"},WorkflowIdReusePolicy:{REJECT_DUPLICATE:"REJECT_DUPLICATE"},ApplicationFailure:{nonRetryable:(_m:string,c:string)=>Error(c)}}));
