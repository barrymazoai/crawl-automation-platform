...
export function prepare(inputRaw: unknown, outputRaw: unknown, storageId: string) {
    ...
    const key = `operations/${input.operationId}/${input.inputFingerprint}`;
    ...
    const ref = (suffix: string, data: Buffer) => ({ schemaVersion: 1 as const, artifactId: `${suffix}-${digest(input.operationId)}`,
    ...
    return { record: validateRecord({ schemaVersion: 1, storageId, input, result: ref("result", resultBytes), completion: ref("completion", completionBytes) }), resultBytes, completionBytes };
}
