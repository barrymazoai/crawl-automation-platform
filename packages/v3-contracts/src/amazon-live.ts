...
export const AmazonProductJobSchema=z.strictObject({codec:z.literal('amazon-product-job/1'),discovery:CatalogDiscoverySchema,sessionId:ExecutionIdSchema,operationId:ExecutionIdSchema,queues:z.strictObject({capture:VersionTagSchema,plan:VersionTagSchema,file:VersionTagSchema,label:VersionTagSchema,review:VersionTagSchema,enrich:VersionTagSchema.optional()}),resources:ResourceGateSchema}).superRefine((j,c)=>{if(j.discovery.scope.channel!=='amazon'||j.discovery.entry.kind!=='product'||j.discovery.entry.variantId!==null||!j.resources.activities.browserSession)c.addIssue({code:'custom',message:'Selected ASIN and exclusive browser phase required'});});
