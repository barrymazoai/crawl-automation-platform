...
export const AmazonRenderedProductSchema=z.strictObject({codec:z.literal('amazon-rendered/1'),url,canonicalUrl:url,capturedAt:z.iso.datetime(),asin,title:z.string().trim().min(1).max(4000),brandRaw:z.string().max(1000),storeUrl:url.nullable().default(null),
 commerce:CommerceEvidenceSchema.optional(),originalHtml:ArtifactRefSchema.refine(v=>v.kind==='source-html'&&v.byteSize<=6*1024*1024).optional(),deliveryText:z.string().max(1000),galleryCount:z.number().int().min(1).max(100),gallery:z.array(z.strictObject({index:z.number().int().min(0).max(99),url,alt:z.string().max(4000)})).min(1).max(100),
