...
import { ProductEvidenceJoinSchema, SavedProductWorkflowInputSchema } from "./product-evidence.js";
...
export const ChannelLabelPlanResultSchema = z.strictObject({ input: ChannelLabelInputSchema, manifest: SavedProductWorkflowInputSchema.shape.manifest, imageOrder:z.array(ExecutionIdSchema).max(100).optional() });
...
export const ChannelSingleLabelSelectionSchema=z.strictObject({input:ChannelLabelInputSchema,selectedImageId:ExecutionIdSchema.nullable(),
 states:z.array(z.union([ProductEvidenceJoinSchema.shape.states.element,z.strictObject({id:ExecutionIdSchema,status:z.literal("not_started")})])).max(100)
}).refine(r=>r.input.evidencePolicy==="label-image-first/5");
export const ChannelLabelImageCheckSchema=z.strictObject({input:ChannelLabelSourceRequestSchema,complete:z.boolean()});
